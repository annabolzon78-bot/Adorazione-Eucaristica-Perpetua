...
router.get('/', async (req, res) => {
  try {
    const dioceses = await prisma.diocese.findMany({
      ...
      include: { country: { select: { nameIt: true, flagEmoji: true } }, city: { select: { name: true } }, _count: { select: { parishes: true } } },
      orderBy: { name: 'asc' },
    })
...
router.get('/:id', async (req, res) => {
  try {
    const diocese = await prisma.diocese.findUniqueOrThrow({
      ...
      include: { country: true, city: true, parishes: { select: { id: true, name: true, hasAdoration: true } } },
    })
...
router.patch('/:id', authenticate, isAdmin, async (req, res) => {
  try { return R.ok(res, await prisma.diocese.update({ where: { id: req.params.id }, data: req.body })) }
  catch { return R.notFound(res, 'Diocesi non trovata') }
})
